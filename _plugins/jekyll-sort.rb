  ...
  class DataSorter < Jekyll::Generator
    ...
    def generate(site)
      ...
      sort_jobs.each do |job|
        ...
        elsif job.has_key?('include_categories')
          data = filter_by_category(config[job['src']], job['include_categories'])
        else 
          data = config[job['src']]
        end
    ...
    # Filter content collection by the "include_categories" attribute on the config
    #   +content_collection+ an array of hashes
    #   +categories+ an array of categories to filter by 
    #
    # Returns a filtered collection by tag
    def filter_by_category(content_collection, categories)
      filtered = []
      content_collection.each { |item|
        categories.each do |category|
          if (item["categories"] && item["categories"].include?(category)) || (item["categories"] && item["categories"].include?(category.capitalize))
            filtered.push(item)
            next
          end
        end
